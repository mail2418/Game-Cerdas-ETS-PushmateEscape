...
public class ExperimentControl : MonoBehaviour
{
    ...
    {
        Objective = 18;
        LevelObjective = LevelToObjectiveMap[18];
        LevelGame = 1;
        ...
        NarrativeDisplay.text = "Freedom Force !";
    ...
    private void ResetScenario()
    {
        ...
        for (int subobjI = 0; subobjI < SubObjectiveAchieved.Length; subobjI++) SubObjectiveAchieved[subobjI] = true; 
        PartialRewards = 0.4f; 

        DecisionCounter = 0; 

       LevelTextDisplay.text = "LEVEL: " + LevelGame.ToString();
...
/*
        ...
        if(Objective>1)
        {
            ...
            for (int subobjI = 1; subobjI < Objective; subobjI++)
            {
                if(!SubObjectiveAchieved[subobjI])
                {
                    if(ReviewCurrentLevelObjective(subobjI))
                    {
                        PartialRewards = PartialRewards + 0.3f;
        ...
        CurrentLevelAchieved = ReviewCurrentLevelObjective(Objective);
        // ====================================
        if (CurrentLevelAchieved)
        {
            ...
            if ((Objective >= 17) && (BothOnUpperCheck())) PartialRewards = PartialRewards + (Prisoner1.transform.localPosition.x - 4.0f) / 5.0f + (Prisoner2.transform.localPosition.x - 4.0f) / 5.0f;

            // If 4: Currently at CrateTaWall Assign a +1.0f additional Reward  and If 18: priosners Escaped Add a +5.0f Reward 
            if (Objective == 4) PrisonerAgentsGroup.AddGroupReward(1.0f + PartialRewards - 0.1f * (float)DecisionCounter / MaxNumberOfDecisions);
            else if(Objective == 18) PrisonerAgentsGroup.AddGroupReward(5.0f + PartialRewards - 0.1f * (float)DecisionCounter / MaxNumberOfDecisions);
            else if (Objective > 18) PrisonerAgentsGroup.AddGroupReward(4.8f + PartialRewards - 0.1f * (float)DecisionCounter / MaxNumberOfDecisions);
            ...
            if (ManualTest)
            {
                if(Objective < 19) Objective = Objective + 1;
            } 
            else
            {
                // Review the LevelObjectiveCount and Increment the Experiment Level
                if ((PromotionCount >= PromotionThreshold) && (Objective<18))
                {
                    Objective = Objective + 1;
                    PromotionCount = 0;
                    DemotionCount = 0;
                    Debug.Log(" A Promotion to Level: " + Objective.ToString() + " Occured at Training Step: " + Academy.Instance.TotalStepCount.ToString());
                }
        ...
        if (DecisionCounter > MaxNumberOfDecisions)
        {
            ...
            if (DemotionCount >= DemotionThreshold)
            {
                ...
                PromotionCount = 0;
                DemotionCount = 0;
                Debug.Log(" ** An Environment Has Just been Demoted Down To: " + Objective.ToString()+  "  Occured at Training Step: " + Academy.Instance.TotalStepCount.ToString()); 
            }

            //PrisonerAgentsGroup.AddGroupReward(PartialRewards - 2.0f - 0.25f* Objective);       // I think we should Explicitly Penalise Running Out of Time
